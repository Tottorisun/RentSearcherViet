...
listings/. Шаги Facebook и batdongsan сначала вставляли и лишь потом
...
из listings/ и свой журнал, а журналы у сервера и ПК разные. Блок
...
не видя серверных строк, ПК выдал бы те же номера.
    ...
    why = repo_sync.prepare(["listings"])      # до allocate и вставки
    ...вставка партии path...
...
def commit_and_push(files, msg, batch=None, redo=None):
    """После правки listings/. None -- в репозитории, иначе что не получилось.
    ...
    # Перебазировать свой коммит бесполезно: insert_listings дописывает строки в
    # конец файла города, и если сервер дописал в тот же файл, git видит конфликт
    # (так было и с единым rebuild_final.py до 17.09.2026, при любом совпадении по
    # времени). Коммит снимается, listings/ возвращается к общему виду, а партия -- она лежит на диске отдельным
    ...
    git("reset", "-q", "HEAD~1")
    git("checkout", "--", SOURCE)
    # Файл города, который вставка создала впервые, после reset стал неотслеживаемым:
    # без этого повторная вставка нашла бы в нём свои же строки и отказала бы.
    git("clean", "-fdq", "--", SOURCE)
    p = _pull()
    if p.returncode:
