...
Что проверяется и что нет:
  ...
  fbmarketplace, fbgroup -- ЗДЕСЬ НЕ ПРОВЕРЯЮТСЯ, и вот измеренная причина.
    На любой запрос без сессии Facebook отвечает HTTP 400 и служебной страницей
    «Error» примерно в 1542 байта. Одинаково -- на живой пост, на снятый, на
    выдуманный и на Marketplace: проверено 8 сентября 2026 на пяти ссылках,
    включая заведомо живой пост 2227162764523235 и заведомо снятый
    28698912193035115. Два ответа совпадают на 99.09%, и вся разница -- токен
    из 5-7 случайных символов, меняющийся от запроса к запросу. Сигнала нет
    вовсе: проверка по такому ответу либо снесла бы ВСЕ фейсбучные строки,
    либо не тронула бы ни одной. С 14.09.2026 их проверяет ПК входом владельца --
    remove_gone_facebook.py, шаг «Facebook: снятые посты» прогона ПК. Скрипт печатает,
    сколько таких строк осталось непроверенными, чтобы дыра была видна.
    ИЗНУТРИ залогиненной страницы они проверяются -- см. fb_liveness_check.py,
    это отдельная процедура с участием человека.
...
# Facebook отвечает HTTP 400 одинаково на живое, снятое и несуществующее
# (см. подробности в docstring) -- сигнала нет, поэтому даже не пытаемся.
...
def main():
    ...
    if skipped:
        print("НЕ проверяется здесь: %d строк Facebook (%s) — на запрос без сессии он "
              "отвечает HTTP 400 одинаково на живое и на снятое, различить нельзя; "
              "их проверяет ПК входом владельца: remove_gone_facebook.py"
              % (len(skipped), ", ".join(sorted(BROWSER_ONLY))))
