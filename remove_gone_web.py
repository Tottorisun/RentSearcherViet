...
Что проверяется и что нет:
  ...
  fbmarketplace, fbgroup -- ЗДЕСЬ НЕ ПРОВЕРЯЮТСЯ, и вот измеренная причина.
    ...
    либо не тронула бы ни одной. Их проверяет ручной заход через залогиненный
    браузер (facebook_check_prompt.txt, fb_groups_howto.md). Скрипт печатает,
...
def main():
    ...
    if skipped:
        print("НЕ проверяется здесь: %d строк Facebook (%s) — на запрос без сессии он "
              ...
              "только ручным заходом через залогиненный браузер"
