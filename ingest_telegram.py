...
# С 13.09.2026 -- и vung-tau, da-lat: у них есть границы, а
# 13.09 посты Facebook с улицей (Thi Sách, Nguyễn An Ninh) отсеивались как «район
# не определяется». Прежде чем добавить, правило сверено со строками Chợ Tốt и
# batdongsan, где район известен: Вунгтау 34 совпадения на 1 расхождение, Далат
# 31 на 2. Фантхьета и Биньзыонга здесь нет: длинная дорога через несколько
# районов (Võ Nguyên Giáp, Mỹ Phước - Tân Vạn) нарезана в OSM неровно, и 7 из 8
# отрезков в одном районе выдают ей ложный район -- 4 и 3 расхождения. Куинён и
# Хойан расхождений не дали, но строк с улицей у них пять и четыре -- мало.
# Выгрузка сделана refresh_osm с рамкой города по его границам; рамка записана в
# самом файле, поле bbox.
OSM_STREET_CITIES = ("da-nang", "ha-noi", "vung-tau", "da-lat")
...
def numbered(by_key):
    """{"phuong 6": "cl", "p 6": "cl", "p6": "cl", ...} для прежних номерных кварталов.
    has_words сравнивает по границе слова: «phuong 1» не находится в «phuong 12»."""
    return {form % n: key for key, nums in by_key.items() for n in nums
            for form in ("phuong %d", "p %d", "p%d")}
...
WARD_ALIASES = {
    ...
    # Прежние НОМЕРНЫЕ кварталы (15.09.2026): посты Далата и Вунгтау пишут «Nguyễn An
    # Ninh, Phường 6, Đà Lạt», «Ngô Đức Kế P7» -- в Далате так делали 6 из 13 постов
    # Facebook на диске. Номер внутри города один, и каждый вошёл в новый район
    # целиком: резолюции 1671/NQ-UBTVQH15 (Лам Донг) и 1685/NQ-UBTVQH15 (Вунгтау,
    # «nhập nguyên trạng»). Второй, независимый источник у каждого номера -- данные:
    # перевод Chợ Tốt ward_name -> ward_name_v3 (около 70 объявлений Далата, 29
    # Вунгтау) и адреса ссылок batdongsan («phuong-11_1» -> xt); расхождений ноль.
    # Phường 12 Далата (Lâm Viên) и Phường 4 Вунгтау (Vũng Tàu) подтверждены только
    # статьёй -- не взяты. Именованные прежние кварталы Вунгтау (Nguyễn An Ninh,
    # Thắng Nhất...) не взяты тоже: «Nguyễn An Ninh» -- ещё и улица.
    "da-lat": numbered({"xh": (1, 2, 3, 4, 10), "cl": (5, 6), "lb": (7,), "lv": (8, 9), "xt": (11,)}),
    "vung-tau": numbered({"vtp": (1, 2, 3, 5), "tth": (7, 8, 9), "rd": (10,), "pth": (11, 12)}),
}
