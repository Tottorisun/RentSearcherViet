...
# Посты в группах Facebook даты публикации машиночитаемо не отдают: в payload её
# нет, а метки на странице принадлежат комментариям и соседним постам вперемешку
# -- 7 сентября 2026 пять постов из трёх групп дали одну и ту же «дату», что и
# выдало подделку. Поэтому у них daysAgo означает не возраст объявления, а время
# с проверки: каждый пост открывается по прямой ссылке и подтверждается живым.
DATELESS_SOURCES = {"dotproperty", "fbmarketplace", "fbgroup"}
