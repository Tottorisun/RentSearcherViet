...
====================================
  * Сервер Netcup (188.68.56.188) -- всё, кроме Facebook и Telegram, с
    публикацией. Служба и таймер лежат в server/ (05:00 и 17:00 по Вьетнаму),
    обёртка начинает каждый прогон с origin/main. Компьютер владельца ночью
    выключен, а сайт должен быть свежим к утру.
  * ПК владельца -- `--candidates-only` (задача планировщика через
    run_daily_program.ps1): группы Facebook и каналы Telegram, без публикации.
  * Сессия, которая заводит строки из кандидатов на ПК, начинает с `git pull`:
    сервер пушит дважды в сутки.
