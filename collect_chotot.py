...
Хойан, Далат, Вунгтау, Куинён, Фантьет, Фукуок и Бинь Зыонг, а следом добавлен
Нячанг -- всего десять. Коды провинций и районов лежат в TARGETS.
...
TARGETS = [
    ...
    # Нячанг включён 9 сентября 2026, когда в CITIES добавили четыре укрупнённых
    # района реформы 2025 года. До этого площадка отдавала «Phường Bắc Nha Trang»
    # и подобные, а у нас лежали только двенадцать прежних -- совпадений не было
    # ни одного, и город не собирался вовсе.
    ("nha-trang",   "7044",  ["704401"]),
    # Города с ежедневными авиарейсами, добавлены 12 сентября 2026. Коды районов
    # сняты не из справочника (в нём только провинции), а из самих объявлений:
    # различные area_v2 в ответе по провинции. Берутся только ГОРОДСКИЕ районы --
    # в провинции Cần Thơ, например, сидят ещё Ô Môn, Cờ Đỏ и Phong Điền, и с
    # ними на страницу города поехали бы деревни.
    ("can-tho",     "5027",  ["502701", "502703", "502704"]),
    ("hai-phong",   "4019",  ["401901", "401902", "401903", "401905"]),
    ("hue",         "6040",  ["604001"]),
    ("buon-ma-thuot", "9053", ["905301"]),
]
...
def describe(ad, type_ru, ward, city_ru, city_en):
    ...
    # Регистр не гарантирован: продавцы пишут и «Đường số 2», и «đường số 2».
    street = re.sub(r"^đường\s+", "", nfc(ad.get("street_name")), flags=re.I)
    street = re.sub(r"^số\s+", "№ ", street, flags=re.I)
...
def batch_text(rows, ids, days):
    ...
    doc = [
        ...
        'пересказом текста продавца, возраст не больше %.1f дней по последней публикации или' % days,
        'поднятию объявления (list_time).',
        '',
...
def main():
    ...
    ap.add_argument("--city", default=None, help="взять только один город сайта")
        ...
        for area in areas:
            for cat, type_ru in CATEGORIES.items():
                ...
                for ad in data.get("ads", []):
                    ...
                    # Возраст -- от ПОСЛЕДНЕЙ публикации или поднятия объявления, а не
                    # от первой. Правило владельца 14.09.2026: хозяин, который снова
                    # выложил или поднял объявление, ищет жильца, и мы показываем его
                    # ещё 7 дней. Раньше бралась первая публикация, если она была: у 491
                    # объявления Chợ Tốt (замер 14.09) она старше недели, а поднято
                    # объявление за последние дни, и на сайт оно не попадало, хотя у
                    # 3372 объявлений без первой даты возраст и так считался по поднятию.
                    ts = max(ad.get("list_time") or 0, ad.get("orig_list_time") or 0) or None
