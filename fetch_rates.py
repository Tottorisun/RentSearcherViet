...
# Скрипт печатает ₫ и ₱, а консоль ежедневного прогона на Windows -- cp1251.
# Без этого он ЗАПИСЫВАЛ rates.json и падал на последней строке вывода,
# отдавая ненулевой код: для вызывающего это неотличимо от провала шага.
...
# Валюты, в которых МОГУТ быть объявлены цены. Доллар добавлен 8 сентября 2026:
# в Тхао Дьене владельцы объявляют аренду в USD, и такая строка на сайте уже
# есть (1000579). До этого он подтягивался только ради строки пересчёта (SHOW),
# то есть работал случайно: убери его из SHOW -- и сборка упала бы на проверке
# «unknown currency», хотя данные корректны.
LISTING = ["VND", "PHP", "USD"]
