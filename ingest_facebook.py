...
цена, фотографии на диске, и район доказан одним из трёх способов:
  ...
  * Дананг: улица из адресной строки найдена в OpenStreetMap, и её отрезки
    ...
    ingest_telegram.py);
  * название из адресной строки уже встречается на сайте не меньше чем в двух
    строках, и все они в одном районе.
...
ПРЕЦЕДЕНТ ИЩЕТСЯ ТОЛЬКО ПО РЕДКИМ СЛОВАМ. Иначе «уликой» становятся «house for
rent», «Cebu City» и «spacious studio»: 12 сентября такое совпадение чуть не
завело строку в Bình Trưng только потому, что два объявления сайта с теми же
общими словами стоят там.
...
def resolve(c, text, city, ctx, exclude):
    ...
    for phrase in named_phrases(lines_, df, nrows, getattr(ctx, "ad_cities", None)):
        ...
        w = it.words(name or "")
        if len(w) < 3 or w in it.CITY_WORDS.get(city, ()) or w in ctx.ward_words.get(city, {}):
            continue
